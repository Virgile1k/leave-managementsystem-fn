...
const DepartmentFormModal = ({ isOpen, onClose, department = null, setActionPerformed }) => {
  const dispatch = useDispatch();
  const { isCreating, isUpdating } = useSelector(selectDepartments);
  ...
  const handleSubmit = (e) => {
    ...
    // Only include headId if it's not empty
    if (formData.headId) {
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={handleInputChange}
                  className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500"
                  placeholder="Enter user ID for department head (optional)"
                />
                <p className="mt-1 text-xs text-gray-500">
                  Leave empty if not assigning a department head
                </p>
              </div>
